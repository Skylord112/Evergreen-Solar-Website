...
if (importBtn) {
	...
	function wpjellyReportError(content) {
		if (content && (content.length > 0)) {
			...
			if (wpjellyControl.plugin_version) {
				content += "\n" + "Plugin Version: " + wpjellyControl.plugin_version;
			}
			...
			if (response.clear_cache)
			{
				...
				jQuery.ajax({
					...
					success: function (final_result) {
						wpjellyMigrateCompleted(response.message)
					},
					error: wpjellyFailedToProcessRequest
				});
			} else {
